...
namespace Equip
{
    public partial class StudentForm : Form
    {
        ...
        public StudentForm(int id)
        {
            InitializeComponent();
            reservationId = id;
            txtSchoolID.KeyPress += TxtSchoolID_KeyPress;
        ...
        private void LoadReservationData()
        {
            if (reservationId > 0)
            {
                ...
                if (reader.Read())
                {
                    ...
                    txtYearSection.Text = reader["YearSection"].ToString();
                    txtSubjectTeacher.Text = reader["SubjectTeacher"].ToString();
                    ...
                    dtpBorrowDate.Value = borrowDate;  // Set only the date part
                    dtpBorrowTime.Value = DateTime.Today.Add(borrowTime);  // Set only the time part

                    DateTime returnDate = Convert.ToDateTime(reader["ReturnDate"]);
                    TimeSpan returnTime = TimeSpan.Parse(reader["ReturnTime"].ToString());
                    dtpReturnDate.Value = returnDate;  // Set only the date part
                    dtpReturnTime.Value = DateTime.Today.Add(returnTime);  // Set only the time part
                }
                reader.Close();
                con.Close();
            }
            ...
            try
            {
                ...
                WHERE EquipmentName = @EquipmentName
                  ...
                  AND (@ReservationId = 0 OR ReservationId != @ReservationId)";
                    ...
                    string query = reservationId > 0
                        ? "UPDATE ReservationTbl SET Date = @Date, Category = @Category, EquipmentName = @EquipmentName, SchoolID = @SchoolID, Name = @Name, Course = @Course, YearSection = @YearSection, Position = @Position, SubjectTeacher = @SubjectTeacher, Quantity = @Quantity, BorrowDate = @BorrowDate, BorrowTime = @BorrowTime, ReturnDate = @ReturnDate, ReturnTime = @ReturnTime WHERE ReservationId = @Id"
                        : "INSERT INTO ReservationTbl (Date, Category, EquipmentName, SchoolID, Name, Course, YearSection, Position, SubjectTeacher, Quantity, BorrowDate, BorrowTime, ReturnDate, ReturnTime) " +
                          "VALUES (@Date, @Category, @EquipmentName, @SchoolID, @Name, @Course, @YearSection, @Position, @SubjectTeacher, @Quantity, @BorrowDate, @BorrowTime, @ReturnDate, @ReturnTime)";

                    SqlCommand cmd = new SqlCommand(query, con);
                    cmd.Parameters.AddWithValue("@Date", DateTime.Now);
                    if (reservationId > 0) cmd.Parameters.AddWithValue("@Id", reservationId);
                    ...
                    cmd.Parameters.AddWithValue("@YearSection", txtYearSection.Text.ToUpper());
                    cmd.Parameters.AddWithValue("@Position", "Student");
                    cmd.Parameters.AddWithValue("@SubjectTeacher", CapitalizeFirstLetter(txtSubjectTeacher.Text));
                    ...
                    cmd.ExecuteNonQuery();
                    MessageBox.Show(reservationId > 0 ? "Reservation updated successfully." : "Reservation created successfully.");
        ...
        private void btnClear_Click(object sender, EventArgs e)
        {
            ...
            dtpReturnTime.Value = DateTime.Now.AddDays(1);
        }
